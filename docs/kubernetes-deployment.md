...
GitHub identity-provider mode is enabled from the integration detail view, but activation is login-first: the UI redirects through GitHub login, the callback creates or updates the Identity user, and the API only activates the provider after it can grant that user `AuthorizedUser`. If login fails, activation is rejected and the integration remains disabled. When an identity provider is enabled, anonymous users are redirected to provider login before the management UI is shown. Set `config.managementAuthEnabled=true` to require `AuthorizedUser` on mutating management APIs. After bootstrap, authorized users create one-time invite links on the Users page; invite codes are embedded in the link, stored only as hashes, redeemed automatically after provider login, and expire according to `config.managementAuthInviteCodeExpiration`. Signed-in users without `AuthorizedUser` are redirected away from the management UI to a standalone invite-code page. External accounts are ASP.NET Core Identity users linked through `AspNetUserLogins`, with GitHub using provider `GitHub` and the GitHub numeric user id as the provider key.
