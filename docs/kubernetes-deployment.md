...
For Kubernetes access, prefer the in-cluster runner service account and grant it only the permissions required for Helm to manage the Formicae release. When the chart manages Pod Security Admission for DinD, the installer must also be allowed to create the chart's short-lived ClusterRole and ClusterRoleBinding and bind the namespace-specific `get`/`patch` permission. If namespace Pod Security is managed separately by a cluster administrator, set `agentJobs.dind.configureNamespacePodSecurity=false` and keep the installer namespace-scoped. If the runner is outside the cluster, provide a repository secret named `FORMICAE_KUBECONFIG_B64` containing a base64-encoded kubeconfig. Do not commit kubeconfigs, cluster API URLs, tokens, hostnames, webhook secrets, OAuth secrets, Codex auth files, or runtime connection strings.
...
When DinD is enabled, the chart defaults `agentJobs.dind.configureNamespacePodSecurity=true`. A pre-install/pre-upgrade Helm hook labels the configured `config.kubernetesJobsNamespace`, or the release namespace when no override is set, with `pod-security.kubernetes.io/enforce=privileged`; `audit` and `warn` remain at `baseline` so privileged workloads are visible to cluster operators. The target namespace must already exist when it differs from the release namespace. The hook receives temporary `get` and `patch` access to that exact namespace and Helm removes the hook Job, ServiceAccount, ClusterRole, and ClusterRoleBinding after success or failure. Disable this setting only when the cluster already exempts or configures the agent-job namespace. Because Pod Security Admission is namespace-scoped, every trusted workload in the target namespace can then request privileged settings.
