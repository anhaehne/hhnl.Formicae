...
public static class WorkflowNodeDefinitions
{
    ...
    public static WorkflowDefinitionValidationResult Validate(WorkflowDefinitionDocument document)
    {
        ...
        void Error(string message, string path = "steps") => errors.Add(new("definition.node.invalid", message, path));
        ...
        if (!nodes.TryGetValue(document.StartStepId, out var start) || start.Uses == TriggerUses) Error("Manual start must reference a task or loop node.", "startStepId");
        foreach (var node in nodes.Values)
        {
            if (node.NextStepId is { } next && (!nodes.ContainsKey(next) || nodes[next].Uses == TriggerUses)) Error($"Node '{node.Id}' has an invalid next connection.");
            if (node.NextStepPort is not (null or "return")) Error($"Node '{node.Id}' has an unknown target port.");
            if (node.Uses is TriggerUses or LoopUses)
            {
                if (node.NextStepId is null) Error($"Control node '{node.Id}' needs an outgoing connection.");
                if (node.NextStepPort is not null) Error($"Control node '{node.Id}' cannot connect to a Return input.");
                if (node.AiSettingsId is not null || node.Model is not null) Error($"Control node '{node.Id}' cannot have agent settings.");
                if (node.Uses == TriggerUses && (node.Trigger is null || node.Loop is not null)) Error($"Trigger '{node.Id}' needs trigger settings only.");
                if (node.Uses == LoopUses && (node.Loop is null || node.Trigger is not null)) Error($"Loop '{node.Id}' needs loop settings only.");
            }
            else if (!WorkflowDefinitionValidator.TryMapUsesToTaskKind(node.Uses, out _) || node.Trigger is not null || node.Loop is not null)
                Error($"Task '{node.Id}' has an unsupported type or control settings.");
        }
        if (errors.Count > 0) return new(errors);
        ...
        foreach (var loop in nodes.Values.Where(n => n.Uses == LoopUses))
        {
            ...
            {
                body.Add(cursor);
                if (!owner.TryAdd(cursor, loop.Id)) Error($"Loop '{loop.Id}' overlaps another loop.");
            ...
            if (body.Count == 0 || nodes[body[^1]].NextStepId != loop.Id || nodes[body[^1]].NextStepPort != "return")
                Error($"Loop '{loop.Id}' needs a nonempty, sequential task body ending at its Return input. Nested loops are not supported.");
            if (loop.NextStepId == loop.Id || body.Contains(loop.NextStepId!)) Error($"Loop '{loop.Id}' must exit outside its body.");
            bodies[loop.Id] = body;
        }
        foreach (var node in nodes.Values)
        {
            if (node.NextStepPort == "return" && (node.NextStepId is null || !bodies.TryGetValue(node.NextStepId, out var returnBody) || returnBody.LastOrDefault() != node.Id))
                Error($"Only the last body task can connect to a loop's Return input ('{node.Id}').");
            if (node.NextStepId is { } target && owner.TryGetValue(target, out var loopId))
            {
                var body = bodies[loopId];
                var index = body.IndexOf(target);
                if (index == 0 || body[index - 1] != node.Id) Error($"Enter loop '{loopId}' through its node, not through a body task.");
            }
        }
        ...
        foreach (var id in nodes.Keys.Except(reached)) Error($"Node '{id}' is not reachable from a manual or trigger entry.");
        if (errors.Count > 0) return new(errors);
