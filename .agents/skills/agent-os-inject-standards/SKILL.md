...
description: Use when the user wants relevant Agent OS standards read into context or referenced for implementation, skill creation, or planning.
---
...
$agent-os-inject-standards api/response-format api/auth  # Multiple files
$agent-os-inject-standards root                          # All standards in the root folder
$agent-os-inject-standards root/naming                   # Single file from root folder
```
...
2. **Creating a Skill** — Building a `Codex skills` file
3. **Shaping/Planning** — In plan mode, building a spec, running `$agent-os-shape-spec`

**Detection logic:**

- If currently in plan mode OR conversation clearly mentions "spec", "plan", "shape" → **Shaping/Planning**
- If conversation clearly mentions creating a skill, editing `Codex skills`, or building a reusable procedure → **Creating a Skill**
- Otherwise → **Ask to confirm** (do not assume)

**If neither skill nor plan is clearly detected**, Ask the user directly to confirm:
...
```
No standards index found. Run $agent-os-discover-standards first to create standards,
or $agent-os-index-standards if you have standards files without an index.
```
...
Match index descriptions against the context. Ask the user directly to present suggestions:
...
First, Ask the user directly to determine how to include the standards:
...
When in conversation scenario, check if `Codex skills` exists and contains related skills:
...
- **Check the index** — If suggestions seem wrong, run `$agent-os-index-standards` to rebuild
- **Keep standards concise** — Injected standards consume tokens; shorter is better

## Integration

This command is called internally by `$agent-os-shape-spec` to inject relevant standards during planning. You can also invoke it directly anytime you need standards in context.
