...
description: Use when the user wants to create or update Agent OS product docs for mission, roadmap, and tech stack in this repository.
---
...
- **Always ask the user directly** and wait for their answer before continuing when a decision is required
...
**If any files exist**, use direct user question:
...
After they respond, use direct user question:
...
**If the tech-stack standard exists**, read it and use direct user question:
...
**If no tech-stack standard exists** (or they chose option 2 above), use direct user question:
...
You can edit them directly or run $agent-os-plan-product again to update.
```
...
- The `$agent-os-shape-spec` command will read these files when planning features, so having them populated helps with context
