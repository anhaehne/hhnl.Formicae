...
description: Use when the user wants to rebuild or maintain agent-os/standards/index.yml from project standards files.
---
...
The index enables `$agent-os-inject-standards` to suggest relevant standards without reading all files. It maps each standard to a brief description for quick matching.
...
2. Ask the user directly to propose a description:
...
- If `$agent-os-inject-standards` suggestions seem out of sync
- To clean up a messy or outdated index

**Note:** `$agent-os-discover-standards` runs this automatically as its final step, so you usually don't need to call it separately after discovering standards.
