...
The manual start contract remains `POST /api/workflows/github-issue`. Workflow definition versions can also include DevOps issue-label triggers. When a connected GitHub or Gitea repository sends an `issues/labeled` webhook matching a configured label and selected repository, Formicae starts the same issue workflow path and records an audited trigger event linked to the workflow run.

The API hosts the background orchestration loop. It wakes automatically on supported GitHub and Gitea webhooks and also polls periodically while the process is running.
...
When enabling a GitHub identity provider, the UI first sends the current browser through GitHub login. The API only activates the provider after that callback has produced an authenticated Identity user, and grants that user the `ManagementAdmin` role in the same operation. If authentication fails, provider activation is rejected and the integration remains unchanged. When any identity provider is enabled, anonymous users are redirected to provider login before the management UI is shown. When `ManagementAuth:Enabled=true`, workflow reads require `WorkflowViewer`, workflow commands require `WorkflowOperator`, and configuration/admin endpoints require `ManagementAdmin`; higher roles imply lower workflow capabilities. Admin users create invite links from the Users page, and redeemed invites grant `ManagementAdmin` for compatibility with the existing invite flow. Invite codes are shown once, embedded into the invite link, stored only as hashes, expire according to `ManagementAuth:InviteCodeExpiration`, and are redeemed automatically after provider login when the link is used. Signed-in users without any management permission are redirected away from the management UI to a standalone invite-code page where they can only enter an invite code or log out.
